...
def find_best_prescaler_and_ocr(desired_cycles):
    ...
    for prescaler in prescalers:
        for TCNT_value in range(0, 255):
            actual_cycles = prescaler * (256 - TCNT_value) + 21
            cycles_diff = abs(actual_cycles - desired_cycles)

            if cycles_diff < best_cycles_diff:
                best_cycles_diff = cycles_diff
                best_prescaler   = prescaler
                best_TCNT_value   = TCNT_value
                best_actual_cycles = actual_cycles

    return best_prescaler, best_TCNT_value, best_actual_cycles, best_cycles_diff

# Example usage
desired_cycles = int(input("Enter the desired number of cycles: "))
best_prescaler, best_TCNT_value, best_actual_cycles, best_cycles_diff = find_best_prescaler_and_ocr(desired_cycles)

print(f"Best Prescaler:  {best_prescaler}")
print(f"Best TCNT Value: {best_TCNT_value}")
